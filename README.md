\page NeutronColl Neutron Collimator

 This project is for studies of the Neutron Generator at the NOvA
 Near Detector with various collimator and neutron detectors.

## GEOMETRY DEFINITION

   The geometry is constructed in the B1::DetectorConstruction class.
   The setup conssists of a hollow cylinder and a detector plane.
...
## ACTION INITALIZATION
   ...
   A newly introduced class, NeutronColl::ActionInitialization, instantiates and registers
   to Geant4 kernel all user action classes.

   While in sequential mode the action classes are instatiated just once,
   via invoking the method:
      NeutronColl::ActionInitialization::Build()
   in multi-threading mode the same method is invoked for each thread worker
   and so all user action classes are defined thread-local.

   A run action class is instantiated both thread-local
   and global that's why its instance is created also in the method
      NeutronColl::ActionInitialization::BuildForMaster()
   which is invoked only in multi-threading mode.

## PRIMARY GENERATOR

   The primary generator is defined in the NeutronColl::PrimaryGeneratorAction class.
...
## DETECTOR RESPONSE
   ...
   in NeutronColl::SteppingAction and accumulated event by event in NeutronColl::EventAction.

   At end of event, the value acummulated in NeutronColl::EventAction is added in NeutronColl::RunAction
   and summed over the whole run (see NeutronColl::EventAction::EndOfevent()).

   Total dose deposited is computed at NeutronColl::RunAction::EndOfRunAction(),
   ...
   workers is merged to the master in NeutronColl::RunAction::EndOfRunAction() and the final
   result is printed on the screen.

   G4Accumulable<G4double> type instead of G4double type is used for the NeutronColl::RunAction
...
## VISUALISATION
   ...
   in the main() function in NeutronColl.cc.
...
## HOW TO RUN
   ...
   - Execute NeutronColl in the 'interactive mode' with visualization
```
% NeutronColl
...
```
   ...
   - Execute NeutronColl in the 'batch' mode from macro files
   (without visualization)
```
% NeutronColl run2.mac
% NeutronColl NeutronColl.in > NeutronColl.out
```
